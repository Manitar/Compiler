...
%define SOB_VOID_ADDRESS const_tbl+" ^ (string_of_int (Code_Gen.find_entry ScmVoid consts_tbl)) ^ "
%define SOB_NIL_ADDRESS const_tbl+" ^ (string_of_int (Code_Gen.find_entry (ScmNil) consts_tbl)) ^ "
%define SOB_FALSE_ADDRESS const_tbl+" ^ (string_of_int (Code_Gen.find_entry (ScmBoolean false) consts_tbl)) ^ "
%define SOB_TRUE_ADDRESS const_tbl+" ^ (string_of_int  (Code_Gen.find_entry (ScmBoolean true) consts_tbl)) ^ "
...
"   ;;; Clean up the dummy frame, set the exit status to 0 (\"success\"), 
   ...
   pop rbp
   add rsp, 3*8 ;Change this back to 3*8
   mov rax, 0
...
try
  ...
  (* let asts = (fun (e) -> [ScmDef'( VarFree "x", ScmConst' (ScmChar '\000'))]) code in *)

  (*let asts = (fun (e) -> [ScmDef' (VarFree "x", ScmConst' (ScmChar '\n'))]) code in *)
  ...
  let code_fragment = String.concat "\n\n"
                        (List.map
                           ...
                           asts) in
(*
  let rec to_num num limit = 
  if num == limit then [num] else [num] @ (to_num (num + 1) limit) in

  let code_fragment = List.fold_left2 (fun code ast count -> code ^ (generate ast count)) "" asts (to_num 1 (List.length asts)) in
*)
